...
class ExtendedKalmanFilterSLAM:
    ...
    def predict(self, control):
        ...
        # compute the new covariance and state.
    ...
    def add_landmark_to_state(self, initial_coords):
        ...
        self.state = concatenate((self.state, list(initial_coords)))
        newCovariance = zeros((len(self.covariance)+2, len(self.covariance)+2))
        newCovariance[0:len(self.covariance), 0:len(
            self.covariance)] = self.covariance
        newCovariance[-2:, -2:] = diag([1e10, 1e10])
    ...
    def correct(self, measurement, landmark_index):
        ...
        H = zeros((2, 3 + 2*n))
        H[:, 0:3] = H3
        H[:, 3+2*landmark_index: 3+2*landmark_index+2] = -1*H3[:, 0:2]
        # H = concatenate((H3, -1 * H3[:, 0:2]), axis=1)
...
if __name__ == '__main__':
    ...
    # Read data.
    logfile = LegoLogfile()
    logfile.read("Unit_F/robot4_motors.txt")
    logfile.read("Unit_F/robot4_scan.txt")

    # Loop over all motor tick records and all measurements and generate
    # filtered positions and covariances.
    # This is the EKF SLAM loop.
    f = open("Unit_F/ekf_slam_correction.txt", "w")
