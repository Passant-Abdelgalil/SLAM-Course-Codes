...
class ExtendedKalmanFilterSLAM:
    ...
    def predict(self, control):
        ...
        # Enlarge G3 and R3 to accomodate all landmarks. Then, compute the
        # new covariance matrix self.covariance.
        n = self.number_of_landmarks
        # compute our new Jacobian G
        G = zeros((2*n+3, 2*n+3))
        G[0:3, 0:3] = G3
        G[3:, 3:]  = eye(2*n)

        # compute our new Jacobian R
        R = zeros((2*n+3, 2*n+3))
        R[0:3, 0:3] = R3
...
if __name__ == '__main__':
    ...
    f = open("Unit_F/ekf_slam_prediction.txt", "w")
